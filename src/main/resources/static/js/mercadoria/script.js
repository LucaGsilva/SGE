...
$(document).ready(function () {
    ...
    function ValidateDescricao() {
        ...
        if ($("#Descricao").val().trim() == '') {
            ...
            document.getElementsByClassName('Mensagem_modal')[0].innerHTML = '<strong>ATENÇÃO !</strong> A descrição deve ser preenchida';
            $('#modal_validate').modal({
                show: true
            })
            $(".modal-backdrop").css("backgroud-color", "transparent");
            $('#Descricao').css("box-shadow", "0 0 0 .2rem rgba(153, 0, 0, 0.445)");
            return false;
        }
        else {
            $('#Descricao').css("box-shadow", "0 0 0 .2rem rgba(0, 0, 0, 0)");
    ...
    function ValidatePreco() {
        ...
        if ($("#Preco").val().trim() == '') {
            ...
            $(".modal-backdrop").css("backgroud-color", "transparent");
            $('#Preco').css("box-shadow", "0 0 0 .2rem rgba(153, 0, 0, 0.445)");
            return false;
        }
        else {
            $('#Preco').css("box-shadow", "0 0 0 .2rem rgba(0, 0, 0, 0)");
